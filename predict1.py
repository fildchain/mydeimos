...
def get_dataframe_from_excel():
    # pd.read_excel()函数用于读取 Excel 文件的数据
    # 'supermarket_sales.xlsx'表示 Excel 文件的路径及名称
    # sheet_name='销售数据'表示读取名为“销售数据”的工作表的数据
    # skiprows=1 表示跳过 Excel 中的第 1 行，因为第 1 行是标题
    ...
    # pd.to_datetime将'时间'列转换成 datetime 类型
...
def add_sidebar_func(df):
    ...
    with st.sidebar:
        ...
        city = st.multiselect(
            ...
            default=city_unique,   # 第 1 次的默认选项为 city_unique
        )

        # 求数据框“顾客类型”列去重复后的值，赋值给 customer_type_unique
customer_type_unique = df["顾客类型"].unique()
customer_type = st.multiselect(
    "请选择顾客类型：",
    options=customer_type_unique,  # 将所有选项设置为 customer_type_unique
    default=customer_type_unique,  # 第 1 次的默认选项为 customer_type_unique
)
...
gender = st.multiselect(
    ...
    default=gender_unique,  # 第 1 次的默认选项为 gender_unique
)

# query()：查询方法，传入过滤条件字符串
# @city: 通过可以使用 Streamlit 多选下拉按钮“城市”的值
# @customer_type: 通过@可以使用 Streamlit 多选下拉按钮“顾客类型”的值
# @gender: 通过@可以使用 Streamlit 多选下拉按钮“性别”的值
# 最后赋值给变量 df_selection
...
def product_line_chart(df):
    ...
    def hour_chart(df):
    # 将 df 按'小时数'列分组，并计算'总价'列的和
    sales_by_hour = (
        df.groupby(by=["小时数"])[["总价"]].sum()
    )
    # 使用 px.bar 函数生成条形图
    # - x="总价"：条形图的长度表示总价
    # - y=sales_by_hour.index：条形图的标签是产品类型
    # - title：设置图表标题，使用 HTML 标签加粗
...
def main_page_demo(df):
    ...
    # 选中数据框中的“总价”列，使用 sum() 计算“总价”列的和，使用 int() 求整
    total_sales = int(df["总价"].sum())
    # 选中数据框中的“评分”列，使用 mean() 计算“评分”列的平均值，使用 round() 四舍五入保留一位小数
    ...
    # 选中的数据框中的“总价”列，使用 mean() 计算“总价”列的平均值，使用 round() 四舍五入保留两位小数
